;;; newer version: Sept. 17 2013 (with eigens)
...
(define-syntax fresh
  (syntax-rules ()
    ((_ (x ...) g0 g ...)
     (lambdag@ (c : B S D Y N T)
       (inc
         (let ((x (var 'x)) ...)
           (let ((B (append `(,x ...) B)))
             (bind* (g0 `(,B ,S ,D ,Y ,N ,T)) g ...))))))))
...
(define c->S (lambda (c) (cadr c)))
(define c->D (lambda (c) (caddr c)))
(define c->Y (lambda (c) (cadddr c)))
(define c->N (lambda (c) (cadddr (cdr c))))
    ...
    (lambdag@ (c : B S D Y N T)
      (cond
        ((unify u v S) =>
         (lambda (S0)
           (cond
             ((==fail-check B S0 D Y N T) (mzero))
             (else (unit `(,B ,S0 ,D ,Y ,N ,T))))))
        (else (mzero))))))
